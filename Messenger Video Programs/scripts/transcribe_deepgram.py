...
def print_dashboard():
    while True:
        ...
        print("\n" + "="*80)
        print(f"   DEEPGRAM DASHBOARD - {datetime.now().strftime('%H:%M:%S')}")
        print("="*80)
...
def main():
    ...
    for f in mp4_files:
        ...
        json_name = os.path.basename(f).replace(".mp4", "_diarized.json")
    ...
    for video_file in mp4_files:
        json_name = os.path.basename(video_file).replace(".mp4", "_diarized.json")
        ...
        update_status(video_file, "Transcribing")
        ...
        try:
            with open(audio_file, "rb") as buffer:
                ...
                # We need a list of segments with start, end, speaker, text, and words
                
                dg_result = response.results.channels[0].alternatives[0]
                
                # Deepgram returns 'words' with speaker info directly!
                # We can construct segments from paragraphs or utterances
                
                # Let's use paragraphs for cleaner segmentation if available, or utterances
                final_segments = []
