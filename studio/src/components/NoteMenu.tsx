...
function MenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-ed-fg-secondary hover:bg-ed-surface-strong hover:text-ed-fg transition-colors text-left"
    >
      <Icon size={14} weight="regular" className="shrink-0 text-ed-fg-secondary" />
      {label}
    </button>
  )
}
...
function ToggleMenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center justify-between gap-2 px-3 py-1.5 text-xs text-ed-fg-secondary hover:bg-ed-surface-strong hover:text-ed-fg transition-colors text-left"
    >
      <span className="flex items-center gap-2">
        <Icon size={14} weight="regular" className="shrink-0 text-ed-fg-secondary" />
        {label}
      </span>
      <span className={`w-3 h-3 rounded-full border ${checked ? 'bg-ed-accent border-ed-accent' : 'border-ed-rule'}`} />
    </button>
  )
}
  ...
  return (
    <div className="relative">
      <button
        ...
        className="p-1.5 rounded-md text-ed-fg-secondary hover:text-ed-fg hover:bg-ed-surface transition-colors"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-56 bg-ed-surface border border-ed-rule rounded-lg shadow-ed-lg z-50 py-1 animate-fadeInUp">
              ...
              {(['default', 'serif', 'mono'] as FontFamily[]).map((f) => (
                <button
                  ...
                  className={`flex-1 py-1 rounded text-xs transition-colors ${
                    fontFamily === f ? 'bg-ed-surface-strong text-ed-fg font-bold' : 'text-ed-fg-secondary hover:text-ed-fg'
