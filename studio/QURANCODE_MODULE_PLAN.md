...
**9j, The traditional divisions. ✅ SHIPPED. This closes 9g's data gate.**
Tanzil's `quran-metadata.txt` under CC-BY, imported as `divisions.tsv` and `prostrations.tsv`, with `revelation_place` added to the chapter table. Ten new Rust tests.

**The numbering problem was smaller than this plan feared, and now it is measured rather than assumed.** §7 said the metadata "has to be imported first" and would need remapping from 6,236 onto 6,234. In fact sura 9 is the *only* chapter where Tanzil and this corpus disagree, and no boundary of any kind lands past 9:127, so nothing is remapped at all. The generator asserts both facts: exactly one disagreeing chapter and it must be sura 9, and every boundary must resolve to a verse that exists here. What does differ is the content of the six divisions, one of each kind, that span 9:127 to 10:1 — they hold two fewer verses here than a Hafs mushaf gives them. That is inherent to the two numberings rather than a defect, and it is counted and printed at generation and written into `CREDITS.md` so the difference is on the record instead of in someone's head.

**Each kind must tile the corpus, and that is an invariant.** A gap would silently drop verses from a scoped count and an overlap would double them; neither shows up as a wrong-looking number on screen. So the generator checks that every kind's ranges sum to exactly 6,234 verses, that the numbering runs 1..n with no holes, and that the bowing count each sura declares matches the number of bowing rows starting in it. Rust checks the same property from the other side, by inverting the ranges into a per-verse index and asserting all 6,234 verses resolve.

**One picker, not five controls.** §6 lists page, station, part, group, quarter and bowing as five remaining scopes. They are one scope with the kind as a parameter, so `Scope` gains a single `division: Option<DivisionRef>` and the surface gains one dropdown plus a number stepper. The number defaults to whichever division the verse on screen already sits in, so choosing "page" answers a question about where the reader is standing rather than jumping to page 1.

**A division narrows the level rather than replacing it**, which is what makes "this page, inside this sura" expressible, and the scope label says which combination produced a figure.

**An unnumbered Basmalah belongs to no division.** It has no verse number, so it has no address, so it is in no page. Letting it into a page count would put four words on a page the mushaf does not print them on, and would make the Basmalah at 1:1 count twice on page 1. A test asserts that asking for Basmalahs cannot add words to a page.

**Two bugs of the same kind, and the fix is at the writer.** A `Map` keyed by number read with a string key returned `undefined`, which `String()` turned into the literal text "undefined" — a full column of it, emitted without complaint. The same class of mistake had already made a verse-count assertion silently compare zero chapters. `cell()` now fails the build on `undefined` or `null`, so a lookup that returns nothing can no longer be written out as data.
