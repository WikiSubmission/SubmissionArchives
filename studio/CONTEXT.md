...
**The design system is the archive's, not Studio's own.** Colour, type, shape, and motion come from `../DESIGN.md` and the `--ed-*` tokens in `../src/app/globals.css`, mirrored into `src/App.css` (see Phase 7). Studio boots in the light "Illuminated Archival Paper" theme; dark is "Midnight Obsidian". Do not introduce a Studio-only palette, a third accent, or a radius that is not on the 4/6/8/12px scale.
...
### Phase 7: Archive Theme Parity & Panel Workspace (COMPLETED 2026-08-20)

Studio no longer has its own look. It adopts the archive's finalised design system (`../src/app/globals.css`, `../DESIGN.md`), and the workspace is rebuilt on resizable panels taking after [delibae/claude-prism](https://github.com/delibae/claude-prism). The old "Midnight Vault" palette — cold black/grey chrome with an amber accent — is gone.

**Theme: one system, two archives**
- ✅ **`--ed-*` tokens mirrored name for name** from the archive's `globals.css`: "Illuminated Archival Paper" (light, `#FBF9F5` paper, `#B86233` terracotta) and "Midnight Obsidian" (dark, `#0F0E0D`, `#C8794A`). Studio **boots light**, matching the archive's own `:root` default. A value changed in one file has to change in the other; that is the cost of the parity and it is deliberate, since the alternative was two drifting palettes for one product.
- ✅ **Tokens Studio adds on top**, because a desktop app needs surfaces the web archive does not: `--ed-success` / `--ed-danger` (+ `-soft`) as warm-harmonised status colours instead of stock Tailwind hues, `--ed-scrim` for modal backdrops, `--ed-on-accent` for ink on an accent fill (it flips: white on light-mode terracotta, dark ink on the lighter dark-mode accent), `--ed-page-sheet` / `--ed-page-ink` for the Page-mode sheet, and `--ed-cat-1..5` for i'rab role colouring on the canvas. The categorical set exists because DESIGN.md's one-accent-per-surface rule governs chrome, not data encoding — five grammatical roles need five distinguishable hues.
- ✅ **Appearance is per machine, not per vault:** [`useAppearance.ts`](src/hooks/useAppearance.ts) owns the `data-theme` attribute on `<html>` and persists to `localStorage`, deliberately *not* to `<archive>/.studio/settings.json` like everything else. Which theme you read in is a property of the screen in front of you; it should not follow the archive onto someone else's machine. Light / Obsidian / System, with a title-bar toggle, `Ctrl+Shift+D`, and a Settings row. `SettingsModal` previously kept its own copy of this state and assumed dark meant "no attribute at all" — that duplicate is gone.
- ✅ **Fonts self-hosted, same five the archive loads** via `next/font`: DM Sans (chrome), Source Serif 4 (display/headings), Newsreader (long-form prose), JetBrains Mono (references, timestamps, status bar), Amiri (Arabic). Latin subsets are the variable builds, so one `.woff2` covers each weight range; total added weight ~115 KB. Still zero CDN calls at runtime, so the 100%-offline charter holds.
- ✅ **One corner-radius system** (closes the open item at the end of Phase 6): Tailwind's own `--radius-*` scale is overridden to DESIGN.md's sharp editorial values — 4 / 6 / 8 / 12 / 16px. Every existing `rounded-*` in the app snapped to the scale without touching a single component, which is the point of overriding the scale rather than auditing call sites.
- ✅ **Quran embed is theme-aware** (extends §4D): in dark it stays the parchment card floating above the chrome, with the wikisubmission.org `LIGHT_PRINT_PALETTE` values verbatim. In light that contrast is not available — a parchment card on paper is invisible — so it becomes a *framed inset* instead: stronger rule (`--qv-border` 0.28 vs 0.18), no elevation (`--qv-shadow: none`). Same sub-brand, different mechanism per theme, driven entirely by tokens.
- ✅ **Contrast audit** (closes the WCAG item at the end of Phase 6): measured every text node in both themes against its composited background. The archive's `--ed-fg-muted` measures 3.2–3.9:1 on dark surfaces and 4.46:1 on the light panel, so it is **not an AA body-text colour in either theme**. Rather than diverge from the archive's palette values, Studio changed which rung it reaches for: `--ed-fg-muted` and `--ed-fg-faint` are now for icons, rules, and decorative numerals, and anything meant to be *read* uses `--ed-fg-secondary`. Re-audit: 0 failures in both themes.

**Panel workspace (prism-inspired)**
- ✅ **[`WorkspaceLayout.tsx`](src/components/workspace/WorkspaceLayout.tsx)** replaces the fixed 250px sidebar and the hand-rolled `SplitPane` drag maths with `react-resizable-panels`: explorer / editor / inspector are drag-resizable and collapsible, and the split view is a nested `PanelGroup`. Sizes persist per archive via `autoSaveId`, so Studio stores no layout state of its own.
- ✅ **[`PaneSwitcher.tsx`](src/components/workspace/PaneSwitcher.tsx)** is prism's layout hover-card, moved: prism keeps it in the sidebar footer, where it vanishes the moment the sidebar collapses. Studio puts it in the title bar so it stays reachable whichever panes are hidden.
- ✅ **The editor is now a hideable pane too** (`Ctrl+\`), with a guard: hiding the last visible pane brings the editor back rather than leaving an empty workspace.
- ✅ **[`ExplorerPanel.tsx`](src/components/workspace/ExplorerPanel.tsx)** — the sidebar tab shell lifted out of `App.tsx`, which was carrying the whole layout tree inline.
- ✅ **Two shortcuts that were wired but dead:** `view.toggle-split` and `view.open-canvas` had handlers in `App.tsx` but no entry in `SYSTEM_COMMANDS`, and `useShortcuts` only walks that list — so their keys did nothing. Registered, along with `view.toggle-editor` and `view.toggle-appearance`.

**Three things that fought the library, and what they cost**
- **Collapse cannot be animated by driving `resize()`.** Prism animates its sidebar width over rAF; that works because prism does not persist pane sizes. Studio does, and the saved layout is reapplied over any imperative resize, so the pane snapped back. Collapse now goes through the library's own `collapse()` / `expand()`, which are the calls it records in that layout.
- **Nor by transitioning `flex-grow`.** The obvious replacement — a CSS transition on the inline `flex-grow` the library writes — left the rendered boxes disagreeing with the library's own layout model: correct grow values (18/82/0), widths of 542/30/663. The panes therefore **snap**, and the only transition is a short opacity fade on pane contents. Worth knowing before anyone tries to re-add a width animation.
- **Every adjacent panel pair needs a mounted handle.** Unmounting the handle beside a hidden pane produces `WARNING: Missing resize handle` and silently breaks imperative collapse/expand. Handles stay mounted and go `inert` (zero width, no pointer events) instead.

**Typography settings were dead config, now wired**
`settings.typography` existed since Phase 6 with no UI and no consumer, and its options named fonts Studio never bundled (`jakarta`, `plex`, a `mushaf` face). [`useTypography.ts`](src/hooks/useTypography.ts) applies the choices by overriding the font tokens at runtime — Tailwind compiles `font-sans` to `var(--font-sans)`, so re-pointing that variable re-points every existing utility. Options now name only bundled families, old vault values migrate onto them, and the Appearance section has the pickers. `'default'` is deliberately absent from the migration map: it meant "whatever the app picks", and the app now picks a different face per role, so it falls through to each field's own default rather than freezing old vaults onto the UI sans for body prose. Editor prose defaults to Newsreader, per-note `fontFamily` frontmatter still overrides.

**Not done in this pass**
- [ ] **Drag-to-resize verified only structurally.** The handles carry the library's own drag behaviour, correct `col-resize` cursor, and a widened hit area, but synthetic `PointerEvent`s do not drive its pointer-capture path, and the preview pane in this environment could not composite frames for a real drag. Worth one manual confirmation in `tauri dev`.
