...
def update_status(filename, status):
    with STATUS_LOCK:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] {filename}: {status}")
...
def main():
    ...
    for audio_file in audio_files:
        ...
        if os.path.exists(output_file):
            # Only skip if transcript is newer than audio
            if os.path.getmtime(output_file) > os.path.getmtime(audio_file):
                continue
            update_status(audio_file, "Re-transcribing (Updated Audio)")
        else:
