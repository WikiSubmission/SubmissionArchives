...
export const metadata: Metadata = {
    title: 'Video Archives — Submission Archives',
    description:
        'Systematic video programs, instructional works, Friday sermons, and conference recordings by Dr. Rashad Khalifa.',
};
...
function getVideoCatalog(): VideoRecord[] {
    const catalog = readJsonIndex(path.join(SOURCE_CATALOG_DIR, 'videos.json')) as VideoRecord[];
    if (catalog.length > 0) return catalog;

    return (readJsonIndex(path.join(GENERATED_DIR, 'MASTER_INDEX.json')) as VideoRecord[]).filter(
        (item) => item.type === 'video-program' || item.type === 'sermon' || item.type === 'video',
    );
}
