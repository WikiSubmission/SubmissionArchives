...
export function WrittenArchiveVisual() {
    return (
        <div className="overflow-hidden border border-ed-rule bg-ed-surface">
            ...
            <div className="bg-[color-mix(in_oklch,var(--ed-surface)_72%,var(--ed-bg))] px-4 pb-0 pt-8 sm:px-6 sm:pt-10">
                <div className="mx-auto flex max-w-xl items-end justify-center">
                    {SHELF_COVERS.map((cover, index) => (
                        <Link
                            key={cover.id}
                            href={`/library/${cover.id}`}
                            aria-label={`Open ${cover.title}`}
                            className={`group relative -mx-2 block w-[22%] max-w-[8.5rem] origin-bottom border border-ed-rule bg-ed-bg shadow-[0_14px_35px_color-mix(in_oklch,var(--ed-fg)_16%,transparent)] transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] first:ml-0 last:mr-0 hover:z-10 hover:-translate-y-3 hover:rotate-0 ${SHELF_TILT[index]}`}
                ...
                <div className="mx-auto h-1.5 max-w-2xl border-x border-t border-ed-rule bg-ed-surface" aria-hidden="true" />
            </div>
