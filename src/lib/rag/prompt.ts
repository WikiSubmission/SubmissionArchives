...
export function buildUserPrompt(
  ...
  return `QUESTION\n${question}\n\nRESEARCH INTENT\n${intent}\n${intentInstruction(intent)}\n\nRETRIEVAL CONFIDENCE\n${retrievalStrength}\n\n${sourceBlocks}\n\nRESPONSE RULES\nGive a direct, readable answer in plain text. Begin by indicating whether the evidence is a direct match, a conceptual match, related but uncertain, or insufficient. When the exact term is absent, explicitly say so before describing the conceptual match. Draw on every supplied source that materially addresses the question, not only the strongest one, and include the relevant specifics each source contributes. When the sources state them, name the concrete particulars rather than paraphrasing them away: specific verse citations, proper names, dates, editions, numbers and quantities, and any secondary points or examples the sources raise alongside the main one. Never introduce a particular the sources do not contain. Organize longer answers into short paragraphs. Use citations in the exact form [S1] or [S1, S2]. Do not include a bibliography.`;
}
